...
class PraxisLLM:
    def __init__(self, mock: bool = False) -> None:
        self._mock = mock
        self._mlflow_endpoint_url = self._get_mlflow_endpoint_url()
        self.prompt_template_builder = InferenceTemplate()

    def _get_mlflow_endpoint_url(self) -> str:
        # Return the MLflow endpoint URL
        # For local deployments, this will be http://localhost:1234/invocations
        # For remote deployments, this would be the URL of the deployed model
        return f"http://localhost:{settings.MLFLOW_SERVE_PORT}/invocations"
    ...
    def generate(
        ...
        if sample_for_evaluation is True:
            add_to_dataset_with_sampling(
                ...
                dataset_name="PraxisLLMMonitoringDataset",
            )
    ...
    def call_llm_service(self, messages: list[dict[str, str]]) -> str:
        ...
        # Call MLflow endpoint instead of SageMaker
        payload = {
            "inputs": {
                "messages": messages,
                "parameters": {
        ...
        try:
            response = requests.post(self._mlflow_endpoint_url, json=payload, timeout=30)
            ...
            if isinstance(result, dict) and "choices" in result:
                ...
                answer = result["outputs"].strip()
            else:
                answer = str(result).strip()
                
            return answer
        except Exception as e:
            logger.error(f"Error calling MLflow endpoint: {e}")
