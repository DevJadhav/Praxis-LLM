...
class RabbitMQPartition(StatefulSourcePartition, Generic[DataT, MessageT]):
    ...
    def __init__(self, queue_name: str, resume_state: MessageT | None = None) -> None:
        ...
        self.last_log_time = time.time()  # Track time for periodic logging
        
        # Ensure the queue exists
        try:
            self.channel.queue_declare(queue=self.queue_name, durable=True)
            logger.info(f"Successfully connected to queue: {self.queue_name}")
        except Exception as e:
            logger.error(f"Failed to declare queue {self.queue_name}: {str(e)}")
    ...
    def next_batch(self, sched: Optional[datetime] = None) -> Iterable[DataT]:
        try:
            ...
            logger.error(
                f"Channel closed by broker while fetching from queue: {str(e)}",
                queue_name=self.queue_name
            )
            time.sleep(10)
            self.connection.connect()
            self.channel = self.connection.get_channel()
            # Redeclare the queue after reconnecting
            self.channel.queue_declare(queue=self.queue_name, durable=True)
            return []
        except Exception as e:
            logger.error(
        ...
        if method_frame:
            ...
            self.last_log_time = time.time()  # Reset log timer when we receive a message
            return [json.loads(body)]
        else:
            # Log status message every 5 seconds when no messages
...
class RabbitMQSource(FixedPartitionedSource):
    ...
    def build_part(
        ...
        return RabbitMQPartition(queue_name=settings.RABBITMQ_QUEUE_NAME, resume_state=resume_state)
